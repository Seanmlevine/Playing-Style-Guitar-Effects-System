...
class MusicDealer:
    def __init__(self, weight_path, model, size=128): # mel-size 128 was 64
        self.model = model
        self.index = size
        self.remove = 0  # changed from 15 to 0 to prevent removal of the first and last 15 s
    ...
    def audio_loader(self, audio_path):
        ...
        # y = y[self.remove*Para.sample_rate: -1*self.remove*Para.sample_rate] # removed to for smaller audio files
        ...
        chunk_num = int(y.shape[0] / self.index) # was 64 for previous version
    ...
    def get_genre(self, audio_path):
        ...
        tag_list = dict()
        idx_list = [x for x in range(8)] # range changed from 10 to 8 for testing
        for idx in idx_list:
            tag_list[idx] = 0

        for i, data in enumerate(self.audio_loader(audio_path)):
            with torch.no_grad():
                data = torch.FloatTensor(data).view(1, 1, self.index, self.index)  # resize to fit it in model was 256 x 256 or 1,1, self.index, self.index
